...
{	
	...
	int j;
	int op=0,cl=0;
	if(expression[0]=='*'||expression[0]=='/'||IsOperator(expression[a-1])){ return "postfix is not possible";}
	else{                                      
	...
	return "postfix is not possible";
	}
}
...
int main() 
{
	...
	else{cout<<"postfix is not possible";}
	if(postfix=="postfix is not possible" || l<=2){cout<<"no result";}
	else{
